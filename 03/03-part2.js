...
const drawPath = (text, wireNumber, allCoordinatesTouched, intersectionsArray) => {
  ...
  pathInstructions.forEach((instruction) => {
    ...
    let numSpaces = instruction.substring(1, instruction.length) * 1;
    ...
    while (numSpaces > 0) {
      ...
      if (allCoordinatesTouched[xCoord] === undefined) {
        allCoordinatesTouched[xCoord] = {}
      }
      if (allCoordinatesTouched[xCoord][yCoord] === undefined) {
        allCoordinatesTouched[xCoord][yCoord] = {};
        allCoordinatesTouched[xCoord][yCoord][wireNumber] = numStepsTaken;
      } else if (!allCoordinatesTouched[xCoord][yCoord].hasOwnProperty(wireNumber)) {
        allCoordinatesTouched[xCoord][yCoord][wireNumber] = numStepsTaken;
        let totalStepsArr = Object.values(allCoordinatesTouched[xCoord][yCoord]);
        let totalSteps = totalStepsArr.reduce( (a,b) => a + b, 0);
        intersectionsArray.push([xCoord, yCoord, totalSteps]);
      }
    }
  })
...
let allCoordinatesObj = {};
let allIntersectionsArr = [];

drawPath(wireOne, 'wireOne', allCoordinatesObj, allIntersectionsArr);
drawPath(wireTwo, 'wireTwo', allCoordinatesObj, allIntersectionsArr);

console.log(allCoordinatesObj)
console.log(allIntersectionsArr)

const findShortestManhattanDistance = (allIntersectionsArr) => {
  let minDistance = true;
  allIntersectionsArr.forEach((intersection) => {
    let manhattanDistance = Math.abs(intersection[0]) + Math.abs(intersection[1]);
    if (minDistance === true && manhattanDistance !== 0) {
      minDistance = manhattanDistance;
    } else if (manhattanDistance < minDistance && manhattanDistance !== 0) {
      minDistance = manhattanDistance;
    }
  })
  return minDistance;
...
findShortestManhattanDistance(allIntersectionsArr);

const findMinSteps = (allIntersectionsArr) => {
  let minSteps = allIntersectionsArr[0][2];
  allIntersectionsArr.forEach((intersection) => {
    if (intersection[2] < minSteps) {
      minSteps = intersection[2]
    }
  })
  return minSteps;
}

findMinSteps(allIntersectionsArr);